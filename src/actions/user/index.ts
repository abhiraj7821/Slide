...
import { createUser, findUser, updateSubscription } from "./quires"
...
export const onBoardUser = async ()=>{
    ...
    try{
        ...
        if(found){
            if(found.integrations.length>0){
                ...
                if(days < 5 ){
                    ...
                    const update_token = await updateIntegration(refresh.access_token,new Date(expires_date),found.integrations[0].id)
        ...
        console.log("Error At On Boarding",error);
        return {
            status:500,
        }
...
export const onUserInfo = async () => {
    ...
    try {
        ...
        return { status:404 , message:"bug at onUserInfo"}
    } catch (error) {
        console.log("Error at onUserInfo",error);
        return {status:500,message:"Error at onUserInfo"}
    }
}
